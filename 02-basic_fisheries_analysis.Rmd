...
landings_data %>% ## Start with the landings data frame
  ...
  ggtitle("Length histogram of Caesio cuning in the catch\nLength at 95% maturity shown as a red line.") + ## Add figure title
...
landings_data %>% ## Start with the landings data frame
  ...
  facet_wrap(~Gear) ## This tells the figure to plot by all different gear types, known as facetting
```
...
Next, let's plot a time series of daily landings data. To do this, we first need to tell R that the "Date" column represents actual dates so that it can work with this format. We next group the data frame by date, so that we can next summarize the summed weight caught on each date. We then plot this using ggplot.
...
landings_data %>% ## Start with the landings data frame
  ...
  summarize(daily_landings = sum(Weight_g/1000)) %>% ## For each date, sumamrize the sum of all daily landings
